...
public class ImagePreprocessor {
	...
	private Point[][] findEdgePoints(BufferedImage edgeImage) {
		...
		foundComponents = new Point[foundComponentCount][];
		for (int c = 0; c < foundComponentCount; c++) {
			int key = components.firstKey();
			foundComponents[c] = new Point[components.get(key).size()];
			components.get(key).toArray(foundComponents[c]);
	...
	public double[][][] process(BufferedImage image) {
		...
		Point[][] points = findEdgePoints(edgeImage);
		coefficients = new double[foundComponents.length][][];
		for (int c = 0; c < foundComponents.length; c++) {
			coefficients[c] = fourierTransform(points[c], image.getWidth(),
					image.getHeight());
		}
		return coefficients;
	}
